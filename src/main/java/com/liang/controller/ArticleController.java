...
public class ArticleController {
	...
	public String setArticle(@RequestParam("photo") MultipartFile file, Article2 article2, HttpSession session,HttpServletRequest request)
		...
		//获取项目在容器中的实际发布运行的根路径，如：C:\Program Files\Apache Software Foundation\Tomcat 8.5\webapps\bbs-ssmv2.0\
		projectname = request.getSession().getServletContext().getRealPath("/");

		//把最后的\去掉，如：C:\Program Files\Apache Software Foundation\Tomcat 8.5\webapps\bbs-ssmv2.0
		projectname=projectname.substring(0,projectname.length()-1);

		//取出项目的名称，如bbs-ssmv2.0
		...
		System.out.println("图片的路径"+filePath);
